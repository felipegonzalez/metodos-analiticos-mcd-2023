...
estimar $p(y|t)$ con un modelo simple. No es un modelo "mecanístico", ni nos
sirve para aprender otras cosas, pero
sabemos que da una estrategia de identificación para el efecto causal
que nos interesa. Nótese también que puede haber variables sobre las que estratificamos que pueden producir más o menos variabilidad, aún cuando no se creen caminos no causales.
...
3. No queremos activar caminos no causales al condicionar.
...
comienzan con $T\to$).
Finalmente, nuestro criterio también implica que no condicionamos a 
colisionadores, pues estos ocurren en caminos de la forma $T\to U_1 \cdots V\gets Y$, y esto activa caminos entre $T$ y $Y$ 
que inciden en $T$.
...
resumen
```
La estimación del cruce simple es casi 50% más grande que
el tamaño verdadero del efecto.
...
particular usada (aunque podríamos usar bootstrap para incluir esta incertidumbre
adicional):
...
de $d$-separación e independencia condicional (si condicionamos y $Z$ y $Y$ están $d$ separadas).

- La regla 2 es el criterio de la puerta trasera.
...
Esta es como el criterio de ajuste por puerta trasera,
y reescribimos como:

$$ p_m(y|do(z), x, w) = p_m(y|z, x, w)$$
Para que podamos sustituir $z$ en lugar de $do(z)$ requerimos
que, dado $W$,  no haya caminos activos de puerta trasera de $Z$ a $Y$ en $G_m$.
Por lo tanto, basta examinar la gráfica donde quitamos de $G_m$
las aristas que salen de $Z$, que no pueden crear puertas traseras. Si $Y$ y $Z$ están
$d$-separados por $W$ y $X$ en esta nueva gráfica, no hay caminos de puerta trasera y
podemos sustituir $do(z)$ por $z$.


**Regla 3** Ignorar intervenciones:

Si $Z$ y $Y$ están $d$-separadas por $X$ y $W$ en la gráfica
$G_m$ donde además quitamos cualquier arista a $Z$ si $Z$ no es antecesor de $W$ en $G_m$.

Entonces:

$$ p(y|do(x), do(z), w) = p(y|do(x), w)$$
Quitando el operador $do(x)$:

$$p_m(y|do(z), x, w) = p_m(y|x, w)$$

Consideramos dos casos: 

Si $Z$ es antecesor de $W$, y $Y$ y $Z$ están $d$-separados en $G_m$,
entonces condicionar por $W$, por $d$-separación tenemos:

- Si hay rutas causales de $Z$ a $Y$, estas son bloqueadas por $W$.
- No puede abrir ninguna ruta si hay colisión en $W$.

Así que cualquier manipulación de $Z$ no tiene efecto en la distribución de $Y$.

Si $Z$ no es antecesor de $W$, y $Y$ y $Z$ están $d$-separados en $G_m$ menos
las aristas que llegan a $Z$, condicionar por $W$, por $d$-separación tenemos:

- No puede haber rutas causales de $Z$ a $Y$ (pues ninguna de ellas llega a $Z$)
- No puede abrir ninguna ruta por colisión en $W$ (pues $W$ tendría que ser descendiente de $Z$).

Así que cualquier manipulación de $Z$ no tienen efecto en la distribución de $Y$
...
$p(c|a, f), p(a|f) y $p(f)$. Estos son modelos *estadístico* con el 
...
- Posible discriminación estructural por las carreras que escogen las personas. En este caso, quizá por ejemplo los departamentos de ingeniería son más fáciles, pero tradicionalmente no se alienta a las mujeres escoger ese tipo de carreras. Este efecto está mediado por la selección de departamento.
...
de hombres y mujeres. Si resumimos con la marginal de los departamentos
seleccionados, obtenemos:

```{r}
sims_contraste <- ajuste$draws("contraste_directo", format = "df")
ggplot(sims_contraste, aes(x = contraste_directo)) +
  geom_histogram()
```

El efecto directo no es conclusivo, y tiende a favorecer a mujeres cuando
promediamos sobre la distribución de departamentos seleccionados.

En algún momento de la discusión, este argumento fue mostrado para
decir que no había discriminación en Berkeley específicamente orientado
hacia las mujeres, aún cuando quedaba por investigar por qué
el contraste de tasas de aceptación es tan diferente en cada departamento
(puede haber muchas razones). Sin embargo, otros investigadores
apuntaron al hecho de que la selección de departamentos y la tasa de aceptación 
puede tener una causa común: por ejemplo, la habilidad general.
...
es posible extraer en este caso el efecto directo. Por ejemplo (McElreath),
una hipótesis que se ha planteado es la siguiente:
...
de ser un ambiente discriminatorio para mujeres:
...
existe una componente de discriminación "estructural".
...
ayudan a entrar en cualquier departamente, pero que la probabilidad
...
datos_lista <- c(datos_lista,
  ...
  eta = matrix(c(c(0, 0, 0, 0, 0, 0), c(0, 0, 0, 0, 3, 0)), nrow= 2, byrow = T))
)
...
má ventaja para mujeres. Esto es porque el departamento 5 absorbe a la
mayoría de mujeres de habilidad alta, lo que implica que la habilidad promedio
de mujeres en otros departamentos es más baja que la de los hombres.
