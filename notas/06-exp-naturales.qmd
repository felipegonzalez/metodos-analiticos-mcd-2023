...
$$\lim_{x\to x_0^+} E[Y|X=x] - \lim_{x\to x_0^-} E[Y|X=x]$$
...
mlda_tbl <- read_csv("../datos/mlda.csv") |> 
  select(agecell,  over21, all, homicide, suicide, mva, drugs, external, externalother) |> 
  pivot_longer(cols=c(all:externalother), names_to = "tipo", values_to = "mortalidad") |> 
  filter(tipo %in% c("mva", "suicide", "homicide"))
head(mlda_tbl)
