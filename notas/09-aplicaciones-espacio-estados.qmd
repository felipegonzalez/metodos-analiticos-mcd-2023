...
library(lubridate)
mex_2018_tbl <- 
  read_csv("../datos/wiki_encuestas_2016_en_campaña.csv") |> 
...
Ahora consideramos $mu_i$. Esta cantidad dependerá de:

- La preferencia $p(t_i)$ de los votantes al tiempo $t_i$ que se tomó
la encuesta.
...
$$y_t^1 - \hat{y}_t^0$$ 
Esta cantidad tiene una distribución, que representa lo que sabemos
del efecto causal individual sobre la región tratada $A$. Nótese que
no podemos generalizar a efectos sobre otras regiones: su respuesta
podría ser diferente a la de $A$.
...
#| fig.width: 5
#| fig.height: 3
x_mat <- datos_tbl |> select(x, x_int) |> as.matrix()
x_mat <- cbind(1, x_mat)
modelo <- AddLocalLevel(list(), datos_tbl$y) 
ajuste <- bsts(y ~ x_int, state.specification = modelo, niter = 2500, 
               data = datos_tbl |> select(y, x_int), ping = 10000, 
               prior = SpikeSlabPrior(x_mat[, c(1,3)], 
                                      y = datos_tbl$y, 
                                      expected.model.size = 2))
ajuste$coefficients[,"x_int"] |> qplot() +
  xlab("Efecto de intervención")
```
Y vemos que el intervalo es muy amplio, y que tenemos que asumir
que el efecto es fijo en el tiempo. Esto no es tan apropiado para
la mayoría de las intervenciones, en donde su efecto varía en el tiempo.
...
```{r}
mod_esp <- AddLocalLevel(list(), y_mod)
ajuste_pre <- bsts(y_mod ~ x, mod_esp, niter = 1000, ping = 1000)
```

```{r}
preds_cf <- predict(ajuste_pre, horizon = 30, newdata = x[71:100])
```
...
```{r}
media_dia_post <- apply(preds_cf$distribution, 1, mean)
diferencia <-  mean(y[71:100]) - media_dia_post
quantile(diferencia, c(0.05, 0.5, 0.95)) |> round(2)
```

O más simplemente:

```{r}
CausalImpact(bsts.model = ajuste_pre,
